...
namespace Zorbus\GalleryBundle\Model;
...
class BlockGalleryConfig extends BlockConfig
{

    public function __construct(AdminInterface $admin, FormFactory $formFactory, $httpKernel)
    {
        parent::__construct('zorbus_gallery.block.gallery', 'Gallery Block', $admin, $formFactory);
        $this->enabled = true;
        $this->themes = array(
            'ZorbusGalleryBundle:Block:gallery' => 'Default'
            );
    ...
    public function getFormMapper()
    {
        return $this->formMapper
                        ->add('gallery', 'entity', array(
                            'class' => 'Zorbus\\GalleryBundle\\Entity\\Gallery',
    ...
    public function getBlockEntity(array $data, BlockEntity $block = null)
    {
        ...
        $block->setCategory('Gallery');
        $block->setParameters(json_encode(array('gallery_id' => $data['gallery']->getId())));
